...
$("#predict-button").click(async function () {
	...
	$("#prediction-list").empty(); // clear the list befor appending new list
	...
	let top5 = Array.from(predictions).map(function (p, i) { // this is Array.map
			return {
				...
				classLink: TARGET_LINKS[i] // Get the "Read More" link for each class
			};
				
			
		}).sort((a, b) => b.probability - a.probability)
		.slice(0, 1); // No to display the Classes in the List
	

		// Append the file name to the prediction list
		//var file_name = 'sample.jpg';
		const fileList = $("#image-selector").prop('files');
		let file_name = fileList.length > 0 ? fileList[0].name : 'sample.jpg';
		$("#prediction-list").append(`<li class="w3-text-blue fname-font" style="list-style-type:none;">File name: ${file_name}</li>`);
		...
		top5.forEach(function (p) {
			...
			//$("#prediction-list").append(`<li style="list-style-type:none;">${p.className}: ${(p.probability * 100).toFixed(2)}%</li>`);
        ...
        const classLink = p.classLink;
        $("#prediction-list").append(
            `<li style="list-style-type:none;">
                There is a ${probability}% chance you may have ${className} skin cancer. 
                <a href="${classLink}" target="_blank" style="color: blue;">Read More About ${className.split(",")[0]}</a>
            </li>`);
...
$("#image-selector").change(async function () {
	...
	//model_processArray(fileList); //this line gives output atomatic after uploading image
